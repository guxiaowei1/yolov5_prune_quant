...
from models.common import DetectMultiBackend, Bottleneck
  ...
  #  print("prune module :",model_list.keys())
    ...
    for bnname, bnlayer in model.named_modules():
        if isinstance(bnlayer, nn.BatchNorm2d):
            ...
            weightbndict[bnname] = bn_module.weight.data.abs()
            # print("mask:",mask)
    ...
    pruned_model = ModelPruned(maskbndict=maskbndict, weightbndict=weightbndict, cfg=pruned_yaml, ch=3).cuda()
    ...
    pruned_model.model[-1].anchors = model.model[-1].anchors
    # =============================================================================================== #
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        for si, pred in enumerate(out):
            ...
            # labels = labels[labels[:, -1] > 8]
            # pred = pred[(pred[:, 3] - pred[:, 1]) > 6]
